...
func (a *OpenAIAdapter) Forward(ctx context.Context, requestID string, req *ChatRequest, apiKey string) (int, []byte, http.Header, io.ReadCloser, error) {
	...
	httpReq, err := newValidatedRequestWithContext(ctx, http.MethodPost, url, body)
	if err != nil {
		return 0, nil, nil, nil, err
	}
...
// BuildStreamChunk wraps text in an OpenAI-compatible SSE delta chunk.
func (a *OpenAIAdapter) BuildStreamChunk(text string) []byte {
	type delta struct {
	...
	b, err := json.Marshal(map[string]any{
		...
		"choices": []choice{{Delta: delta{Content: text}, Index: 0, FinishReason: &stop}},
	})
	if err != nil {
		return nil
	}
