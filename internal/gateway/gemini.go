...
func (a *GeminiAdapter) Forward(ctx context.Context, requestID string, req *ChatRequest, apiKey string) (int, []byte, http.Header, io.ReadCloser, error) {
	...
	httpReq, err := newValidatedRequestWithContext(ctx, http.MethodPost, reqURL, body)
	if err != nil {
		return 0, nil, nil, nil, err
	}
...
// BuildStreamChunk wraps text in a Gemini candidates SSE payload.
func (a *GeminiAdapter) BuildStreamChunk(text string) []byte {
